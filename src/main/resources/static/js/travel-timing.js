...
function renderAirportOptions(type, airports) {
  ...
  if (!airports || airports.length === 0) {
    airportArea.innerHTML = `<div class="text-muted small">등록된 공항이 없습니다.</div>`;
    return;
  }

  airports.forEach((airport, index) => {
    const wrapper = document.createElement("label");
    wrapper.className = "airport-option d-block border rounded p-2 mb-2";

    const checked = index === 0 ? "checked" : "";

    wrapper.innerHTML = `
      <input type="radio" name="${type}AirportCode" value="${airport.airportCode}" ${checked}>
      <span class="ms-2">
        <strong>${airport.airportName}</strong>
        <span class="text-muted small">${airport.airportCode}</span>
      </span>
    `;
...
function resetAirportArea(type, message) {
  const airportArea = getAirportArea(type);
  airportArea.innerHTML = `<div class="text-muted small">${message}</div>`;
}
...
function applyQueryParams() {
  ...
  const stayDays = params.get("stayDays");
...
function renderMonthCards(container, months, type) {
  ...
  months.forEach((month) => {
    ...
    col.innerHTML = `
      <article class="result-month-card">
        <div class="result-month-card-header">
          <div>
            <h3>${month.monthLabel}</h3>
          </div>
          <span class="grade-badge ${getGradeClass(month.grade)}">${month.grade}</span>
        </div>

        <div class="score-line">
          <strong>${month.totalScore}</strong>
          <span>/ 100점</span>
        </div>
        ...
        <div class="score-breakdown">
          <div class="score-mini-box">
            <span>항공권</span>
            <strong>${month.score.flightScore} / 80점</strong>
          </div>
          <div class="score-mini-box">
            <span>환율</span>
            <strong>${month.score.exchangeScore} / 20점</strong>
          </div>
        </div>
        ...
        <ul class="month-card-list">
          ...
          <li>
            <span>이벤트</span>
            <strong>${month.eventCount || 0}개</strong>
          </li>
        </ul>
        ...
        <button type="button" class="btn btn-outline-primary month-detail-button" data-month="${month.month}">
          상세 보기
        </button>
      </article>
    `;
...
function renderMonthlyTable(months) {
  ...
  months.forEach((month) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <strong>${month.totalScore}</strong>점
        <div class="score-detail-text">${month.grade}</div>
      </td>
      <td>
        ${month.score.flightScore} / 80
        <div class="score-detail-text">
          TOP5 ${month.score.flightCheapDateScore} /
          최저가 ${month.score.flightMinPriceScore} /
          안정성 ${month.score.flightStabilityScore}
        </div>
      </td>
      <td>
        ${month.score.exchangeScore} / 20
        <div class="score-detail-text">
          평균 ${month.score.exchangeAverageRateScore} /
          데이터 ${month.score.exchangeDataCountScore}
        </div>
      </td>
      <td>${formatPrice(month.minRoundTripPrice)}</td>
      <td>${formatPrice(month.cheapTop5AveragePrice)}</td>
      <td>${formatRate(month.averageRate)}</td>
      <td>${month.eventCount || 0}개</td>
      <td>${month.cheapestWeekdayName || "-"}</td>
      <td>${month.expensiveWeekdayName || "-"}</td>
    `;
...
function renderMonthDetail(month) {
  ...
  renderDateList(cheapDateList, month.cheapestDates || [], "cheap");
  renderDateList(expensiveDateList, month.expensiveDates || [], "expensive");
  renderEventList(eventList, month.events || [], month);
}

function renderDateList(container, dates, type) {
  container.innerHTML = "";
  ...
  dates.forEach((date) => {
    ...
    item.innerHTML = `
      <div class="date-item-title">
        ${date.departureDate} 출발 → ${date.returnDate} 귀국
      </div>
      <div class="date-item-meta">
        ${date.dayOfWeekName} 출발 / 왕복 ${formatPrice(date.totalPrice)}
        <br>
...
function renderEventList(container, events, month) {
  ...
  if (previewEvents.length === 0) {
    ...
    previewEvents.forEach((event) => {
      ...
      item.innerHTML = `
        <div class="event-item-title">${event.eventName || "이벤트명 없음"}</div>
        <div class="event-item-meta">
          ${event.eventType || "EVENT"} / ${event.eventArea || event.cityName || "-"}
        </div>
  ...
  if (fullEventUrl) {
    ...
    buttonWrap.innerHTML = `
      <a href="${fullEventUrl}" class="btn btn-outline-primary event-more-button">
        이 달 이벤트 전체 보기
      </a>
    `;
...
function renderEventNames(events) {
  ...
  return `
    <ul>
      ${topEvents.map((event) => `<li>${event.eventName}</li>`).join("")}
    </ul>
  `;
}
